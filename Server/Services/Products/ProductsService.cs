...
namespace Ecommerce.Server.Services.Products
{
    public class ProductsService : IProductsService
    {
        ...
        {
            var pageResults = 2f;
            var pageCount = Math.Ceiling((await SearchProduct(searchText)).Count()/ pageResults); 
            var products = await _dbContext.Products.Where(p => p.Title.ToLower().Contains(searchText.ToLower())
